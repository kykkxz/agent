...
|---|---|
| 数据库 | SQLite 3 |
| 文件 | `database/transport-safety-kb/data/transport_safety_kb.sqlite3` |
| 模式版本 | 2` |
| 字符编码 | UTF-8 |
...
当前数据库快照时间为 2026-08-13，已基于新增实际数据确认表结构与记录数一致。数据量会在后续采集闭环中变化，表结构以本规范和
...
`evidence_units.document_id` 和 `documents.raw_path` 回到本地原档。
...
| `metadata_json` | TEXT | NOT NULL | 完整来源配置和扩展元数据 JSON |
...
| `source_id` | TEXT | FK, NOT NULL | 关联 `sources.source_id` |
| `source_uri` | TEXT | NOT NULL | 官方来源 URI |
...
| `authority_level` | TEXT | NOT NULL | 来源权威等级 |
...
| `raw_path` | TEXT | NOT NULL | 对应本地原档相对路径 |
...
| `source_uri` | TEXT | NOT NULL | 引用来源 URI 快照 |
| `locator` | TEXT | NOT NULL | 引用位置快照 |
...
| `idx_documents_authority` | `documents.authority_level` | 权威来源排序 |
...
| `idx_knowledge_version_status` | `knowledge_entries.version_status` | 默认检索版本过滤 |
